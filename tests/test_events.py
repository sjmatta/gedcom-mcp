...
class TestCitationDataclass:
    ...
    def test_citation_defaults(self):
        ...
        citation = gs.Citation(source_id="@S1@")
...
class TestEventDataclass:
    ...
    def test_event_to_dict(self):
        """Should include all fields in dict output."""
        citation = gs.Citation(source_id="@S1@", source_title="Test")
        event = gs.Event(
            type="BIRT",
            date="1 JAN 1900",
            place="New York, USA",
...
class TestEventsLoaded:
    ...
    def test_individuals_have_events(self):
        ...
        individuals_with_events = sum(1 for indi in gs.individuals.values() if indi.events)
    ...
    def test_events_have_types(self):
        ...
        for indi in gs.individuals.values():
            for event in indi.events:
                ...
                assert event.type in gs.EVENT_TAGS
...
class TestGetEvents:
    ...
    def test_get_events_for_nonexistent(self):
        ...
        result = gs._get_events("NONEXISTENT999")
        assert result == []
...
class TestSearchEvents:
    ...
    def test_search_returns_list(self):
        """Should return a list."""
        result = gs._search_events()
        assert isinstance(result, list)

    def test_search_by_type(self):
        """Should filter by event type."""
        result = gs._search_events(event_type="BIRT", max_results=10)
        for event in result:
            assert event["type"] == "BIRT"

    def test_search_respects_max_results(self):
        """Should respect max_results parameter."""
        result = gs._search_events(max_results=5)
        assert len(result) <= 5

    def test_search_result_has_individual_info(self):
        """Results should include individual info."""
        result = gs._search_events(max_results=1)
        if result:
...
class TestGetCitations:
    ...
    def test_get_citations_returns_list(self):
        """Should return a list."""
        # Use home person
        result = gs._get_citations(gs.HOME_PERSON_ID)
        assert isinstance(result, list)

    def test_get_citations_for_nonexistent(self):
        """Should return empty list for nonexistent individual."""
        result = gs._get_citations("NONEXISTENT999")
        assert result == []
    ...
    def test_citations_have_event_context(self):
        ...
        for indi in gs.individuals.values():
            for event in indi.events:
                if event.citations:
                    result = gs._get_citations(indi.id)
                    if result:
...
class TestGetNotes:
    ...
    def test_get_notes_returns_list(self):
        """Should return a list."""
        result = gs._get_notes(gs.HOME_PERSON_ID)
        assert isinstance(result, list)

    def test_get_notes_for_nonexistent(self):
        """Should return empty list for nonexistent individual."""
        result = gs._get_notes("NONEXISTENT999")
        assert result == []
...
class TestGetTimeline:
    ...
    def test_get_timeline_returns_list(self):
        """Should return a list."""
        result = gs._get_timeline(gs.HOME_PERSON_ID)
        assert isinstance(result, list)

    def test_get_timeline_for_nonexistent(self):
        """Should return empty list for nonexistent individual."""
        result = gs._get_timeline("NONEXISTENT999")
        assert result == []
    ...
    def test_timeline_is_sorted(self):
        ...
        for indi in gs.individuals.values():
            if len(indi.events) >= 2:
                result = gs._get_timeline(indi.id)
                # Check that years are non-decreasing (events without dates go last)
                years = []
                for event in result:
                    year = gs.extract_year(event.get("date"))
                    if year:
                        years.append(year)
...
class TestEventMCPTools:
    ...
    def test_get_events_function(self):
        ...
        result = gs._get_events(gs.HOME_PERSON_ID)
        assert isinstance(result, list)

    def test_search_events_function(self):
        """Internal function should work."""
        result = gs._search_events(event_type="BIRT", max_results=5)
        assert isinstance(result, list)
